...
<td class="bg-black  text-white" ><center>{{$produc['price']}}.00 TL</center></td>
<td class="bg-black  text-white" ><center>{{$produc['detail']}}</center></td>
<td class="bg-black">
       <form class="justify-flex" action="{{route("products.destroy",$produc['id'])}}" method="POST">
         <a style="white-space: nowrap" class="btn m-2 btn-sm btn-info " href="{{route("products.product_detail",$produc['id'])}}">Görüntüle</a> 
         
         <a class="btn m-2 btn-sm btn-primary" href="{{route("products.show",$produc['id'])}}">Düzenle</a> 
